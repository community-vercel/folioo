...
export default function TestimonialsSlider() {
    ...
    <section className="bg-white py-14">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <Swiper
          ...
          className="pb-12" // Adds space for bullets
        >
          {testimonials.map((t, i) => (
            <SwiperSlide key={i}>
              <div className="bg-white border border-gray-200 shadow-sm rounded-md p-6 h-full flex flex-col justify-between text-center">
                <img
                  src={t.logo}
                  alt={`${t.name} company logo`}
                  className="mx-auto mb-4 h-10 object-contain"
                />
                <p className="text-gray-700 text-sm mb-6 leading-relaxed text-left">
                  {t.content}
                </p>
 ...
 <style jsx global>{`
    ...
    display: block;
          ...
          `}</style>
        {/* Force center swiper pagination bullets */}
